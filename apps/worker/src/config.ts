...
const envSchema = z.object({
  ...
  PRIVACY_DETECTOR_TIMEOUT_MS: z.coerce.number().int().positive().max(60_000).default(20_000),
  PRIVACY_DETECTOR_RETRIES: z.coerce.number().int().min(1).max(6).default(3),
  PRIVACY_DETECTOR_RETRY_BASE_MS: z.coerce.number().int().positive().max(10_000).default(200),
  MEDIA_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(10).default(3),
  MEDIA_STUCK_MINUTES: z.coerce.number().int().positive().max(1440).default(20),
